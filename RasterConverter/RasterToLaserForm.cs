...
namespace LaserGRBL.RasterConverter
{
	public partial class RasterToLaserForm : Form
	{
		...
		private RasterToLaserForm(GrblFile file, string filename)
		{
            ...
            CbMode.Items.Add(ImageTransform.Formula.OpticalCorrect);
            CbMode.SelectedItem = ImageTransform.Formula.OpticalCorrect;
			CbMode.ResumeLayout();
		...
		void BtnCreateClick(object sender, EventArgs e)
		{
            ...
            {
                int direction = 0;
                if (RbDirH.Checked) { direction = 0; }
                else if (RbDirV.Checked) { direction = 1; }
                else if (RbDir45.Checked) { direction = 2; }

                using (Bitmap bmp = ProduceBitmap(false))
                    mFile.LoadImage(bmp, mFileName, (int)UDQuality.Value, IIOffsetX.CurrentValue, IIOffsetY.CurrentValue, IIMarkSpeed.CurrentValue, IITravelSpeed.CurrentValue, IIMinPower.CurrentValue, IIMaxPower.CurrentValue, TxtLaserOn.Text, TxtLaserOff.Text, direction);
			}
			else if (RbVectorize.Checked)
			{
		...
		private void StoreSettings()
		{
            ...
            Settings.SetObject("GrayScaleConversion.Line2LineOptions.DirH", RbDirH.Checked);
            Settings.SetObject("GrayScaleConversion.Line2LineOptions.DirV", RbDirV.Checked);
            Settings.SetObject("GrayScaleConversion.Line2LineOptions.45", RbDir45.Checked);
		...
		private void LoadSettings()
		{
            ...
            RbDirH.Checked = (bool)Settings.GetObject("GrayScaleConversion.Line2LineOptions.DirH", true);
            RbDirV.Checked = (bool)Settings.GetObject("GrayScaleConversion.Line2LineOptions.DirV", false);
            RbDir45.Checked = (bool)Settings.GetObject("GrayScaleConversion.Line2LineOptions.Dir45", false);
        ...
        private void GbVectorizeOptions_Enter(object sender, EventArgs e)
        {
        ...
        private void GbLineToLineOptions_Enter(object sender, EventArgs e)
        {
